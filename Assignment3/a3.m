...
function ret = loss(model, data, wd_coefficient)
  ...
  wd_loss = sum(model_to_theta(model).^2)/2*wd_coefficient; % weight decay loss. very straightforward: E = 1/2 * wd_coefficient * theta^2
  ret = classification_loss + wd_loss;
end

function ret = d_loss_by_d_model(model, data, wd_coefficient)
  % Notice that the loss function is an average over training cases, as opposed to a sum. Of course, that affects the gradient as well.
  ...
  delta = 1/n * (class_prob - data.targets)
  ret.hid_to_class = (hid_output * delta.').' + wd_coefficient .* model.hid_to_class
  anotherdelta = ((model.hid_to_class).' * delta) .* (logistic(hid_input) .* (1-logistic(hid_input)))
  ret.input_to_hid = (data.inputs * anotherdelta.').' + wd_coefficient .* model.input_to_hid
end
